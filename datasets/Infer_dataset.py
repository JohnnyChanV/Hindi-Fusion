...
##Data Preprocess for Classification
    ...
    def __init__(self, config, datatype='test'):
        ...
        super().__init__()
        self.config = config
        self.datapath = f"{config.datadir}/{datatype}.jsonl"
    ...
    def dump_line_data(self,line,dev_tk,rom_tk):
        ...
        line_data = {'rom_sent':[devanagari_to_latin(s) for s in sent],
                     'rom_id':None,
                     'rom_mask':None,
    ...
    #     for line in tqdm(json.load(open(self.datapath,encoding='utf-8'))):
    #         try:
    #             data.append(self.dump_line_data(line,dev_tokenizer,rom_tokenizer))
    #         except:
    #             print(f"[ERROR]: Unsafe DataFormat...{line}")
    #     return data
    ...
    def process_raw_data(self):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                print("[INFO]: Processing raw data...")
                for item in tqdm(json.load(open(self.datapath,encoding='utf-8'))):
...
def Infer_data_loader(config,mode='train',shuffle=True,num_workers=4):
    dataset = InferDataset(config,datatype=mode)
