...
from models.Fusion.Attention import CrossAttention
...
class FeatureFusion(nn.Module):
    def __init__(self,pooling_mode='mean',fusion_mode='cat',embedding_dim=300,usePLM=True,useCLS=False):
        ...
        if fusion_mode=='gate':
            ...
            self.fusion_fc = nn.Sequential(
                nn.Linear(self.sizeBeforePool,self.sizeBeforePool*2),
            ...
            self.attention = CrossAttention(self.sizeBeforePool)

    def fusion(self,dev_embed,rom_embed,fusion_mode='cat'):
        if fusion_mode not in ['cat','mul','gate','add','att']:
        ...
        if fusion_mode=='cat':
            ...
            x = torch.matmul(dev_embed , rom_embed) #(batch,len)
            # x = self.fusion_fc(x)
            return x
            ...
            x = torch.add(dev_embed,rom_embed)
            return x
        elif fusion_mode=='att':
            x = self.attention(dev_embed,rom_embed,torch.add(dev_embed,rom_embed))
            return x
